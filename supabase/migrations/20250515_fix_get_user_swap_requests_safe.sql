...
-- Update the get_user_swap_requests_safe function to fix the ambiguous column reference
...
BEGIN
  ...
  shift_data AS (
    ...
    WHERE s.id IN (SELECT ur.requester_shift_id FROM user_requests ur)
  )
  SELECT 
    r.id,
    r.status,
  ...
  LEFT JOIN shift_data s ON s.id = r.requester_shift_id;
END;
$$;
