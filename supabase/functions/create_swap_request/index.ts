...
serve(async (req) => {
  ...
  try {
    // Parse request data
    const { shift_ids, preferred_dates, required_skillset } = await req.json();
    ...
    // Log the request data including required skillset if present
    ...
    if (required_skillset && Array.isArray(required_skillset) && required_skillset.length > 0) {
      console.log('- Required skillset:', required_skillset);
    }
    ...
    for (const shift_id of shift_ids) {
      ...
      const { data: swapRequest, error: swapRequestError } = await supabaseAdmin
        ...
        .insert({
          ...
          status: 'pending',
          required_skillset: required_skillset && required_skillset.length > 0 ? required_skillset : null
        })
        .select('id')
        .single();
